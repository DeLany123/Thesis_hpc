...
with Observation and Reward Scaling
=========================================================================
...
Observation scaling follows the "both-scaled" approach established in
Chapter 5 (Section normalisation of the thesis):
  - SoC and quarter-energy features are normalised by physical bounds to [0, 1].
  - The imbalance price is scaled with a robust (median / IQR) transform
    fitted exclusively on the training fold, preventing look-ahead leakage.
  - The reward is normalised online using a running estimate of its standard
    deviation (Welford's algorithm), with the direction preserved.
  - During generic training the reward is additionally divided by the current
    battery capacity so the gradient signal stays comparable across episodes
    that sample different battery sizes.

Usage examples::

    python generic_agent_scaled.py --steps 500000
    python generic_agent_scaled.py --steps 500000 --folds-path /data/folds --sequential
    python generic_agent_scaled.py --steps 500000 --cap-min 5 --cap-max 20
    python generic_agent_scaled.py --steps 500000 --price-scale-method standard
...
class GenericBatteryEnv(BaseBatteryEnv):
    ...
    Observation vector (notation follows the mathematical model in Chapter 3):
        ...
        [SoC_norm, price_scaled, A^c_norm, A^d_norm]
    ...
    where E_max_quarter = r * (15 / 60) MWh is the maximum energy exchangeable
    in a single 15-minute quarter at full power, and price_median / price_iqr
    are the robust scaling parameters fitted on the training fold price series.

    The observation bounds are fixed regardless of which (C, r) pair is active,
    enabling the SAC policy to transfer zero-shot to any configuration within
    (or near) the training distribution.

    Reward handling
    ---------------
    During training (normalise_reward=True) the raw EUR reward is divided by
    the current battery capacity so that the gradient signal remains on a
    comparable scale across episodes that differ in absolute EUR reward.
    An outer RunningRewardScaler wrapper further stabilises training by
    normalising by the running reward standard deviation.
    Raw EUR revenue is recovered at evaluation time (normalise_reward=False).
    """
    ...
    def __init__(
        ...
        price_median : float
            Median of the training-fold imbalance price series, used for
            robust observation scaling (fitted on training data only).
        price_iqr : float
            Interquartile range (Q3 - Q1) of the training-fold imbalance
            price series.  Used as the denominator in robust scaling.
        ...
        # capacity so the base class __init__ is satisfied.
        ...
        # Price scaling parameters (fitted on training fold only).
        ...
        # Fixed observation space: bounds do not depend on (C, r).
        # SoC and energy features are in [0, 1]; scaled price is unbounded.
    ...
    def _get_observation(self) -> np.ndarray:
        ...
        Notation follows the mathematical model:
          index 0 — normalised SoC:     SoC_t / C              in [0, 1]
          index 1 — scaled price:        (p_t - median) / IQR   unbounded
          index 2 — normalised A^c_t:   A^c_t / E_max_quarter  in [0, 1]
          index 3 — normalised A^d_t:   A^d_t / E_max_quarter  in [0, 1]
        """
        ...
        # Robust price scaling (training-fold statistics prevent look-ahead).
...
def _run_fold(
    ...
    for run in range(1, n_iterations + 1):
        ...
        # Welford reward scaling applied on top of capacity normalisation.
...
def main():
    ...
    if args.fold is not None:
        ...
        output_path = os.path.join(
            args.output_dir, f"generic_scaled_fold{args.fold}_results.csv"
        )
    ...
    output_path = os.path.join(
        args.output_dir, f"generic_scaled_results_{args.steps}.csv"
    )
    summary_df.to_csv(output_path, index=False)
    print(f"\nResults written to: {output_path}")
    print(summary_df.to_string(index=False))
